...
pub fn debot_translate_getter_answer_impl(
    ...
    let mut msg_info = MsgInfo::create(answer_msg.clone(), j);
...
pub fn debot_build_on_success(
    ...
    build_internal_message(src, dst, new_body.into(), Default::default())
}
...
fn build_answer_msg(
    ...
    let res = build_internal_message(dest_addr, debot_addr, new_body.into(), Default::default());
    Some(res)
}
