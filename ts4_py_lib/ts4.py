...
def red(msg):
    return colorize(BColors.FAIL, str(msg))
...
def process_actions(result, expect_ec = 0):
    ...
    for msg in actions:
        ...
        if msg_type == 'event':
            ...
            if msg_type == 'unknown':
                ...
                assert (msg_type == 'call') or (msg_type == 'empty'), red('Unexpected type: {}'.format(msg_type))
            QUEUE.append(msg)
    return gas
...
def dispatch_one_message(expect_ec = 0):
    ...
    result = process_actions(core.dispatch_message(msg['id']), expect_ec)
    # if msg['id'] == 2050: quit()
    return result
...
def blue(text):
    return colorize(BColors.OKBLUE, text)
...
class ContractWrapper:
    ...
    def call_getter_raw(self, method, params = dict(), private_key = None):
        ...
        j = json.loads(actions[0])
        assert eq('answer', j['type'])
        assert eq(method, j['method'])
        return j['params']
    ...
    def call_method(self, method, params = dict(), private_key = None, expect_ec = 0):
        ...
        assert isinstance(params, dict)
        if G_VERBOSE:
            print(colorize(BColors.OKGREEN, "{} {}".format(method,prettify_dict(params))))
        try:
